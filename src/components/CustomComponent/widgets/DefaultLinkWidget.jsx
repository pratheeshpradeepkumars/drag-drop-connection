...
export class DefaultLinkWidget extends BaseWidget {
	...
	generateLabel(label) {
		...
		return (
			...
			>
				<text fill="white" ref={ref => (this.refLabels[label.id] = ref)}>
	...
	calculateLabelPosition = (label, index) => {
        ...
        labelDimensions.width = 100;
		labelDimensions.height = 50;
		...
		this.refLabels[label.id].setAttribute("x",`${labelCoordinates.x}`);
		this.refLabels[label.id].setAttribute("y",`${labelCoordinates.y}`);
